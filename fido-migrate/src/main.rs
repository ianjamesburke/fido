...
fn query_all_posts(db: &Database) -> Result<Vec<Post>> {
    let conn = db.pool.get().context("Failed to get database connection")?;
...
fn connect_database(path: &str) -> Result<Database> {
    ...
    let db = Database::new(path).context("Failed to open database connection")?;
    ...
    let hashtags_table_exists: bool = conn
        .query_row(
    ...
    let post_hashtags_table_exists: bool = conn
        .query_row(
...
fn display_stats(stats: &MigrationStats, dry_run: bool) {
    ...
    println!(
        "Total hashtag associations: {}",
        stats.total_hashtags_created
    );
...
mod tests {
    ...
    fn test_completeness() {
        ...
        assert_eq!(
            stats.total_hashtags_created, expected_total_hashtags,
            "Total hashtags should match"
        );
        assert_eq!(stats.posts_with_hashtags, 3, "Three posts have hashtags");
        assert_eq!(
            stats.unique_hashtags, 2,
            "Two unique hashtags: rust and webdev"
        );
    }
    ...
    fn test_idempotency() {
        ...
        hashtag_repo
            .store_hashtags(&post1_id, &hashtags1)
            .expect("Failed first migration");
        hashtag_repo
            .store_hashtags(&post2_id, &hashtags2)
            .expect("Failed first migration");

        // Count hashtags after first run
        let count_after_first: i32 = conn
            .query_row("SELECT COUNT(*) FROM post_hashtags", [], |row| row.get(0))
            .expect("Failed to count");

        // Run migration second time (should be idempotent)
        hashtag_repo
            .store_hashtags(&post1_id, &hashtags1)
            .expect("Failed second migration");
        hashtag_repo
            .store_hashtags(&post2_id, &hashtags2)
            .expect("Failed second migration");

        // Count hashtags after second run
        let count_after_second: i32 = conn
            .query_row("SELECT COUNT(*) FROM post_hashtags", [], |row| row.get(0))
            .expect("Failed to count");
    ...
    fn test_association_correctness() {
        ...
        let hashtag_repo = HashtagRepository::new(db.pool.clone());
        hashtag_repo
            .store_hashtags(&post_id, &expected_hashtags)
            .expect("Failed to store hashtags");

        // Query back the hashtags
        let stored_hashtags = hashtag_repo
            .get_by_post(&post_id)
        ...
        for hashtag in &expected_hashtags {
            assert!(
                stored_hashtags.contains(hashtag),
                "Expected hashtag '{}' not found in stored hashtags",
                hashtag
            );
        }
    }
