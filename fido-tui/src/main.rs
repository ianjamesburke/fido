...
async fn load_web_session_file() -> Result<WebSessionData> {
    ...
    // Don't immediately clean up the session file - let it persist for a bit
    // This helps with timing issues in web mode
    log::info!("Successfully loaded web session file");
...
async fn main() -> Result<()> {
    ...
    if let Some(session_token) = cli.session_token {
        ...
        app.api_client
            .set_session_token(Some(session_token.clone()));

        // Validate the session with the server
        match app.api_client.validate_session().await {
            Ok(response) => {
                log::info!(
                    "Successfully validated web session for user: {}",
    ...
    // Check for existing session on startup (skip in web mode without session token)
        ...
        if let Ok(session_data) = load_web_session_file().await {
            log::info!(
                "Found web session file for user: {}",
                session_data.user.username
            );

            // Set the session token in the API client
            app.api_client
                .set_session_token(Some(session_data.session_token));
    ...
    while app.running {
                ...
                if let Some(device_code) = &app.auth_state.github_device_code.clone() {
                        ...
                        .await
                    {
                        Ok(login_response) => {
                            ...
                            if let Err(e) = app
                                .storage_adapter
                                .store_credentials(&login_response.session_token)
                            {
                        ...
                        Err(e) => {
                            ...
                            if !error_msg.contains("authorization_pending") {
                                ...
                                app.auth_state.error =
                                    Some(format!("Device authorization error: {}", e));
                                app.auth_state.github_auth_in_progress = false;
                                app.auth_state.github_device_code = None;
                                app.auth_state.github_user_code = None;
                                app.auth_state.github_verification_uri = None;
        ...
        if app.current_tab == app::Tab::DMs
            && (app.dms_state.selected_conversation_index != last_dm_conversation_index
                ...
                if key.kind == KeyEventKind::Press {
                    ...
                    match key.code {
                        ...
                        KeyCode::Char('g') | KeyCode::Char('G')
                            if app.current_screen == app::Screen::Auth
                        ...
                        KeyCode::Enter if app.posts_state.show_filter_modal => {
                            ...
                            if app.posts_state.filter_modal_state.selected_tab
                                == FilterTab::Hashtags
                        ...
                        KeyCode::Enter | KeyCode::Char(' ')
                            if app.current_tab == app::Tab::Posts
                        ...
                        KeyCode::Enter
                            if app.current_tab == app::Tab::DMs
                                ...
                                && app.input_mode == app::InputMode::Typing =>
                        {
                            app.send_dm().await?;
                        }
                                ...
                                && !app.posts_state.filter_modal_state.show_add_hashtag_input =>
                        {
                            ...
                            if selected_index
                                < app.posts_state.filter_modal_state.hashtag_list.len()
                            {
                                let hashtag_name = app.posts_state.filter_modal_state.hashtag_list
                                    [selected_index]
                                    .clone();
                                ...
                                && !app.friends_state.search_mode =>
                        {
                            ...
                            if let Some(user) = filtered_list.get(app.friends_state.selected_index)
                            {
                                ...
                                let is_following =
                                    app.friends_state.following.iter().any(|u| u.id == user_id);

                                if is_following {
                                    // Unfollow
                                    if let Err(e) = app.api_client.unfollow_user(user_id).await {
                                        app.friends_state.error =
                                            Some(format!("Failed to unfollow: {}", e));
                                    } else {
                                    ...
                                    if let Err(e) = app.api_client.follow_user(user_id).await {
                                        app.friends_state.error =
                                            Some(format!("Failed to follow: {}", e));
                                    } else {
                                        // Reload social connections
                                        app.load_social_connections().await?;
                                    }
                        ...
                        KeyCode::Enter if app.hashtags_state.show_add_hashtag_input => {
                            let hashtag_name =
                                app.hashtags_state.add_hashtag_name.trim().to_string();
                            if !hashtag_name.is_empty() {
                                app.follow_hashtag(&hashtag_name).await?;
                            }
                        }
                        ...
                        KeyCode::Char('s') | KeyCode::Char('S')
                            if app.current_screen == app::Screen::Main
                                && app.current_tab == app::Tab::Settings
                            ...
                            if app.user_profile_view.is_some() =>
                        {
                            ...
                            if let Some(profile) = &app.user_profile_view {
                                ...
                                let is_following = matches!(
                                    ...
                                    app::RelationshipStatus::Following
                                        | app::RelationshipStatus::MutualFriends
                                );
                            ...
                            if app.user_profile_view.is_some() =>
                        {
                            ...
                            if let Some(profile) = &app.user_profile_view {
                                if matches!(
                                    ...
                                    app.open_or_create_dm_conversation(username, user_id_str)
                                        .await?;
                                }
                            }
                        }
