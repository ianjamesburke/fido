...
use fido_server::security::{create_session_cookie, is_https};
...
fn test_http_cookie_without_secure_flag() {
    ...
    assert!(
        !cookie_str.contains("Secure"),
        "Cookie should not have Secure flag for HTTP"
    );
    assert!(
        cookie_str.contains("HttpOnly"),
        "Cookie should still have HttpOnly flag"
    );
    assert!(
        cookie_str.contains("SameSite=Strict"),
        "Cookie should still have SameSite=Strict"
    );
}
...
fn test_cookie_attributes() {
    ...
    assert!(
        cookie_str.contains("session_token=abc123xyz"),
