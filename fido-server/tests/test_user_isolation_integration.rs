...
use fido_server::db::{Database, DatabaseAdapter, IsolatedDatabaseAdapter};
use fido_types::{Post, SortOrder, UserContext, Vote, VoteDirection};
...
async fn test_user_isolation_integration() -> Result<()> {
    ...
    assert_eq!(
        real_posts.len(),
        0,
        "Real users should not see test user posts"
    );
    ...
    assert_eq!(
        test_posts_after_reset.len(),
        0,
        "Test data should be reset to empty"
    );
