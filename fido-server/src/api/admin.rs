...
mod tests {
    ...
    async fn test_validate_config_includes_environment() {
        ...
        assert!(
            response.environment == "development" || response.environment == "production"
        );
    }
    ...
    async fn test_admin_middleware_rejects_invalid_token() {
        ...
        let result = state.session_manager.validate_session("invalid-token-12345");
    ...
    async fn test_admin_middleware_rejects_non_admin_user() {
        ...
        assert!(!user.is_admin, "Regular user should not have admin privileges");
    }
    ...
    async fn test_admin_check_with_expired_session() {
        ...
        assert!(result.is_err(), "Should reject expired session even for admin users");
    }
    ...
    async fn test_config_validation_response_structure() {
        ...
        assert!(response.security.max_request_size > 0, "Should have max request size");
        
        // Verify session config structure
        assert!(response.session.max_age_days > 0, "Should have max age days");
        assert!(response.session.idle_timeout_hours > 0, "Should have idle timeout hours");
