...
mod tests {
    ...
    fn test_seed_test_data() {
        ...
        let conn = db.connection().expect("Failed to get connection");
        let count: i32 = conn
            .query_row("SELECT COUNT(*) FROM users WHERE is_test_user = 1", [], |row| row.get(0))
    ...
    fn test_authentication_migrations() {
        ...
        let mut stmt = conn.prepare("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='sessions'")
            .expect("Failed to prepare statement");
        ...
        assert!(indexes.iter().any(|idx| idx.contains("user_id")), "Should have index on user_id");
        assert!(indexes.iter().any(|idx| idx.contains("expires_at")), "Should have index on expires_at");
        ...
        assert!(user_columns.contains(&"github_id".to_string()), "users table should have github_id column");
        assert!(user_columns.contains(&"github_login".to_string()), "users table should have github_login column");
        
        // Verify unique index on github_id
        let mut stmt = conn.prepare("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='users'")
            .expect("Failed to prepare statement");
        ...
        assert!(user_indexes.iter().any(|idx| idx.contains("github_id")), "Should have index on github_id");
    }
}
