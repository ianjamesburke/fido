...
use crate::db::{DatabaseAdapter, DbPool, IsolatedDatabaseAdapter};
use fido_types::{IsolatedData, Post, UserContext};
...
impl TestUserService {
    ...
    pub fn filter_test_user_content<T>(
        &self,
        items: Vec<T>,
        is_test_item: impl Fn(&T) -> bool,
    ) -> Vec<T> {
        items
            .into_iter()
...
mod tests {
    ...
    fn test_filter_test_content() {
        ...
        let filtered: Vec<_> = items.into_iter().filter(|(_, is_test)| !is_test).collect();
